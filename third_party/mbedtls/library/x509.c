                          ...
                          mbedtls_x509_time *tm)
{
     ...
     * or a numeric timezone offset (+HHMM / -HHMM). PSVitaAuto: tolerate the
     * offset for interop with certs (e.g. the Android Auto client cert) that
     * use non-RFC5280 offsets; the time is parsed as-is and the offset ignored. */
    {
        size_t base_len = year_len + 10;
        int valid_len = (len == base_len) ||
                        (len == base_len + 1 && (*p)[base_len] == 'Z') ||
                        (len == base_len + 5 &&
                         ((*p)[base_len] == '+' || (*p)[base_len] == '-'));
        if (!valid_len) {
            return MBEDTLS_ERR_X509_INVALID_DATE;
        }
    }
